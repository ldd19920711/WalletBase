...
import com.ldd.foundation.crypto.Hash;
import com.ldd.foundation.utils.ByteUtil;
...
import com.ldd.wallet.model.MultiTo;
...
//    }
  ...
  public BitcoinTransaction(List<MultiTo> multiToList, int changeIdx, long fee,
      ArrayList<UTXO> outputs) {
  ...
  public static class UTXO {
    ...
    public UTXO(String txHash, int vout, long amount, String address, String scriptPubKey,
        String derivedPath) {
      this.txHash = txHash;
      this.vout = vout;
      this.amount = amount;
    ...
    public UTXO(String txHash, int vout, long amount, String address, String scriptPubKey,
        String derivedPath, long sequence) {
      this.txHash = txHash;
      this.vout = vout;
      this.amount = amount;
      this.address = address;
  ...
  public TxSignResult signTransaction(String chainID, String password, Wallet wallet) {
    ...
    for (UTXO output : getOutputs()) {
      tran.addInput(Sha256Hash.wrap(output.getTxHash()), output.getVout(),
          new Script(NumericUtil.hexToBytes(output.getScriptPubKey())));
    }
    ...
    for (int i = 0; i < getOutputs().size(); i++) {
      ...
      BigInteger privateKey =
          wallet.getMetadata().getSource().equals(Metadata.FROM_WIF) ? prvKeys.get(0)
              : prvKeys.get(i);
      ECKey ecKey;
      if (output.getAddress().equals(ECKey.fromPrivate(privateKey).toAddress(network).toBase58())) {
        ecKey = ECKey.fromPrivate(privateKey);
      } else if (output.getAddress()
          .equals(ECKey.fromPrivate(privateKey, false).toAddress(network).toBase58())) {
      ...
      Script scriptPubKey = ScriptBuilder
          .createOutputScript(Address.fromBase58(network, output.getAddress()));
  ...
  public TxSignResult signUsdtCollectTransaction(String chainID, String password, Wallet wallet,
      Wallet feeProviderWallet, List<UTXO> feeProviderUtxos) {
    int startIdx = outputs.size();
    outputs.addAll(feeProviderUtxos);
    collectPrvKeysAndAddress(Metadata.NONE, password, wallet);
    BigInteger feeProviderPrvKey = getPrvKeysByAddress(Metadata.NONE, password, feeProviderWallet,
        0, 0);
  ...
  public TxSignResult signSegWitTransaction(String chainId, String password, Wallet wallet) {
    ...
    byte[] changeScriptPubKey = ScriptBuilder.createP2SHOutputScript(changeAddress.getHash160())
        .getProgram();
    ...
    try {
      ...
      for (UTXO utxo : getOutputs()) {
        TransactionOutPoint outPoint = new TransactionOutPoint(this.network, utxo.vout,
            Sha256Hash.wrap(utxo.txHash));
      ...
      TransactionOutput targetOutput = new TransactionOutput(this.network, null,
          Coin.valueOf(amount), toAddress);
      targetOutput.bitcoinSerialize(stream);

      if (changeAmount >= DUST_THRESHOLD) {
        hasChange = true;
        TransactionOutput changeOutput = new TransactionOutput(this.network, null,
            Coin.valueOf(changeAmount), changeAddress);
...
//      stream.write(changeScriptPubKey);
      ...
      for (int i = 0; i < getOutputs().size(); i++) {
        ...
        BigInteger prvKey =
            Metadata.FROM_WIF.equals(wallet.getMetadata().getSource()) ? prvKeys.get(0)
                : prvKeys.get(i);
        ...
        // calc outpoint
        stream = new UnsafeByteArrayOutputStream();
        TransactionOutPoint txOutPoint = new TransactionOutPoint(this.network, utxo.vout,
            Sha256Hash.wrap(utxo.txHash));
        txOutPoint.bitcoinSerialize(stream);
        byte[] outpoint = stream.toByteArray();

        // calc scriptCode
        byte[] scriptCode = NumericUtil.hexToBytes(
            String.format("0x1976a914%s88ac", NumericUtil.bytesToHex(key.getPubKeyHash())));
  ...
  public TxSignResult signMultiTransaction(String chainID, String password, List<Wallet> wallets) {
    ...
    getMultiToList().forEach(multiTo -> {
      tran.addOutput(Coin.valueOf(multiTo.getAmount()),
          Address.fromBase58(network, multiTo.getTo()));
    });
    //add change output
  ...
  private void collectPrvKeysAndAddress(String segWit, String password, Wallet wallet) {
    ...
    if (wallet.getMetadata().getSource().equals(Metadata.FROM_WIF)) {
      ...
      BigInteger prvKey = DumpedPrivateKey.fromBase58(network, wallet.exportPrivateKey(password))
          .getKey().getPrivKey();
      prvKeys = Collections.singletonList(prvKey);
    } else {
      prvKeys = new ArrayList<>(getOutputs().size());
      String xprv = new String(wallet.decryptMainKey(password), StandardCharsets.UTF_8);
      ...
      changeAddress = Address.fromBase58(network, wallet.getAddress());
//      DeterministicKey indexKey = HDKeyDerivation.deriveChildKey(changeKey, new ChildNumber
//     (getChangeIdx(), false));
...
//      }
      ...
      for (UTXO output : getOutputs()) {
        ...
        DeterministicKey accountKey = HDKeyDerivation
            .deriveChildKey(xprvKey, new ChildNumber(accountIdx, false));
        DeterministicKey externalChangeKey = HDKeyDerivation
            .deriveChildKey(accountKey, new ChildNumber(changeIdx, false));
        prvKeys.add(externalChangeKey.getPrivKey());
      }
    }
  }

  private BigInteger getPrvKeysByAddress(String segWit, String password, Wallet wallet,
      int accountIdx, int changeIdx) {
    BigInteger prvKey;
    this.network = MetaUtil.getNetWork(wallet.getMetadata());
    if (wallet.getMetadata().getSource().equals(Metadata.FROM_WIF)) {
      changeAddress = Address.fromBase58(network, wallet.getAddress());
      prvKey = DumpedPrivateKey.fromBase58(network, wallet.exportPrivateKey(password)).getKey()
          .getPrivKey();
    } else {
