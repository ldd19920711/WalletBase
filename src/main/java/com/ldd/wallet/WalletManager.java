...
import com.ldd.wallet.keystore.WalletKeystore;
...
//  }
  ...
  public static Wallet importWalletFromPrivateKey(Metadata metadata, String prvKeyHex,
      String password, boolean overwrite) {
  ...
  public static Wallet importWalletFromMnemonic(Metadata metadata, String mnemonic, String path,
      String password, boolean overwrite) {
    if (metadata.getSource() == null) {
  ...
  public static Wallet findWalletByPrivateKey(String chainType, String network, String privateKey,
      String segWit) {
    if (ChainType.ETHEREUM.equals(chainType)) {
      new PrivateKeyValidator(privateKey).validate();
    }
    Network net = new Network(network);
    String address = AddressCreatorManager.getInstance(chainType, net.isMainnet(), segWit)
  ...
  public static Wallet findWalletByMnemonic(String chainType, String network, String mnemonic,
      String path, String segWit) {
    List<String> mnemonicCodes = Arrays.asList(mnemonic.split(" "));
    MnemonicUtil.validateMnemonics(mnemonicCodes);
    ...
    if (ChainType.BITCOIN.equalsIgnoreCase(chainType) || ChainType.LITECOIN
        .equalsIgnoreCase(chainType)) {
      path += "/0/0";
    }
    ...
    Network net = new Network(network);
    String address = AddressCreatorManager.getInstance(chainType, net.isMainnet(), segWit)
        .fromPrivateKey(key.getPrivateKeyAsHex());
    return findWalletByAddress(chainType, address);
  }
  ...
  private static Wallet flushWallet(IMTKeystore keystore, boolean overwrite) {
    ...
    IMTKeystore existsKeystore = findKeystoreByAddress(keystore.getMetadata().getChainType(),
        keystore.getAddress());
  ...
  private static <T extends WalletKeystore> T unmarshalKeystore(String keystoreContent,
      Class<T> clazz) {
    T importedKeystore;
    try {
   ...
   * @return 压缩后私钥
   */
  public static String bigIntegerToBase58(String hex) {
    if (hex.length() < 64) {
