...
import com.ldd.foundation.crypto.AES;
import com.ldd.foundation.crypto.Crypto;
import com.ldd.foundation.crypto.Hash;
import com.ldd.foundation.crypto.Multihash;
import com.ldd.foundation.utils.ByteUtil;
import com.ldd.foundation.utils.MnemonicUtil;
...
import com.ldd.wallet.keystore.IdentityKeystore;
...
import com.ldd.wallet.transaction.EthereumSign;
import java.io.File;
...
public class Identity {
  ...
  public static Identity createIdentity(String name, String password, String passwordHit,
      String network, String segWit) {
  ...
  void removeWallet(String walletId) {
    ...
    for (; idx < wallets.size(); idx++) {
      ...
      if (wallet != null && wallet.getId().equals(walletId)) {
        break;
      }
    }
  ...
  public List<Wallet> deriveWalletsByMnemonics(List<String> chainTypes, String password,
  ...
  private Wallet deriveBitcoinWallet(List<String> mnemonicCodes, String password, String segWit) {
    ...
    if (Metadata.P2WPKH.equals(segWit)) {
      path = this.getMetadata().isMainNet() ? BIP44Util.BITCOIN_SEGWIT_MAIN_PATH
          : BIP44Util.BITCOIN_SEGWIT_TESTNET_PATH;
    } else {
      path = this.getMetadata().isMainNet() ? BIP44Util.BITCOIN_MAINNET_PATH
          : BIP44Util.BITCOIN_TESTNET_PATH;
    }
  ...
  private Wallet deriveEthereumWallet(List<String> mnemonics, String password) {
    ...
    IMTKeystore keystore = V3MnemonicKeystore
        .create(walletMetadata, password, mnemonics, BIP44Util.ETHEREUM_PATH);
    return WalletManager.createWallet(keystore);
  }
  ...
  private Wallet deriveTronWallet(List<String> mnemonics, String password) {
    ...
    IMTKeystore keystore = V3MnemonicKeystore
        .create(walletMetadata, password, mnemonics, BIP44Util.TRON_PATH);
    return WalletManager.createWallet(keystore);
  }
  ...
  String encryptDataToIPFS(String originData, long unixtime, byte[] iv) {
    ...
    int totalLen = (int) (headerLength + ciphertextLength.getSizeInBytes() + ciphertextLength.value
        + 65);
    byte[] payload = new byte[totalLen];
    int destPos = 0;
    System.arraycopy(toSign, 0, payload, destPos, headerLength);
    destPos += headerLength;
    System.arraycopy(ciphertextLength.encode(), 0, payload, destPos,
        ciphertextLength.getSizeInBytes());
  ...
  public String decryptDataFromIPFS(String encryptedData) {
    ...
    try {
      BigInteger pubKey = EthereumSign
          .ecRecover(NumericUtil.bytesToHex(toSign), NumericUtil.bytesToHex(signature));
      ECKey ecKey = ECKey.fromPublicOnly(ByteUtil
          .concat(new byte[]{0x04}, NumericUtil.bigIntegerToBytesWithZeroPadded(pubKey, 64)));
      String recoverIpfsID = new Multihash(Multihash.Type.sha2_256, Hash.sha256(ecKey.getPubKey()))
          .toBase58();
  ...
  public String signAuthenticationMessage(int accessTime, String deviceToken, String password) {
    ...
    String data = String
        .format(Locale.ENGLISH, "%d.%s.%s", accessTime, getIdentifier(), deviceToken);
